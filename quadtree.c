...
//function for obtaining a radical from a power of 2
int radical(int x)
{
	int sqrt = 2;
...
// knot addition in a quaternary tree
...
// calculating the degree of uniformity of an area of ​​the matrix
...
// creating an image from an array of rgbs
...
/* compression in a quaternary tree of an array of rgbs */
void compress(nod **cap, int x,int y,int l,int width,pixel **grid,int prag,int *nr_of_colors, int *nr_of_nodes)
{	
	// avg => current level of uniformity
	int avg;
	//p => the average color for the current square
	pixel p;
...
/* bringing a quaternary tree to a shape in which
   each level contains fixed 4 ^ level nodes
   the root is considered at level 0 in this case*/
...
/* determining the depth of a quaternary tree */
int Depth(nod* cap)
{
    if (cap == NULL)
        return 0;
...
// function for loading a QuadtreeNode vector
// traversing a tree on levels 
...
/* creating a compression file from a given vector*/
...
/* Reading a vector from a binary file */
...
/* loads a given rgb on a square of the matrix*/
...
/* Load a vector into a quaternary tree*/
...
/* Loading values ​​of a matrix from a tree */
...
/* Loading a matrix with the "averages" of colors on the leaves
   of two quaternary trees with the same number of levels and nodes */
...
void flipV(nod **cap)
{
	...
	{	
		nod *aux = NULL;
		//swap top_left with bottom_left
		aux = (*cap)->top_left;
		(*cap)->top_left = (*cap)->bottom_left;
		(*cap)->bottom_left = aux;

		//swap top_right with bottom_right
		aux = (*cap)->top_right;
...
{	
	...
	//the root of the quaternary tree will be retained in the head
	nod *cap;	
	cap = NULL;
	...
	/* releasing the memory allocated to the requirement 1*/
	...
	{	
		...
		/* freeing the memory allocated to the requirement 2*/
		freeMatrix(grid,width);
		if(v != NULL)
		free(v);
		freeQuadTree(tree);
	...
	{	
		...
		if(strcmp(argv[2],"h")==0)
			...
			rotire = 0;
		//spin == 1 => spin horizontally
		//spin == 2=> spin vertically
	...
	/* releasing the memory allocated to the requirement 3*/
	freeMatrix(grid,width);
	freeQuadTree(cap);
	}
